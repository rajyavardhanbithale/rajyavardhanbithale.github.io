...
export default function Home() {
  ...
  const availableThemes: string[] = ['forest','desert','beach']
  ...
  return (
    <>
      ...
      <div className="flex flex-row gap-5 items-end p-8 justify-center w-full min-h-screen absolute z-50 -mt-36 lg:-mt-0">

        {[...Array(themeCount)].map((_, index) => (
          <div onClick={()=>handleThemeUpdate(index)} key={index} className={`h-2 w-10 rounded-full bg-white ${index===themeActive ? "opacity-100" : "opacity-60"} hover:opacity-100 transition duration-500`}></div>
        ))}
      </div>
